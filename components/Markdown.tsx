...
const components = {
  a: (a) => {
    return (
      <a href={a.href} rel='noopener noreferrer' target='_blank' className='transition-opacity hover:opacity-70'>
        {a.children}
      </a>
    );
  },
  ...
  img: (image) => {
    return <Image src={image.src} alt={image.alt} width={400} height={300} quality={50} layout='responsive' objectFit='contain' objectPosition='center' />;
  },
};
...
const Markdown = ({ content, ...props }) => {
  ...
  return (
    <ReactMarkdown
      ...
      className={classnames(markdownStyles['markdown'], props.className ? props.className : null)}
    >
      {content}
    </ReactMarkdown>
  );
