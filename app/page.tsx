  ...
  return (
    <div>
      <div className="flex w-full flex-col items-center justify-center pt-16">
        <span className={"font-black text-7xl tracking-tighter text-zinc-800 dark:text-zinc-50"}>
          Plugins<span className="text-gray-400">.</span>run
        </span>
      </div>
      <HeaderView>{title}</HeaderView>
      <SubheaderView>
        A series of free utility plugins, downloaded
        <TotalUniqueRuns stats={stats} />
        times, that allow you as a designer or developer to easily manage your day-to-day workflow and improve your efficiencies.
      </SubheaderView>
      <div className="mt-2 grid grid-cols-1 gap-4 px-4 md:grid-cols-2 md:px-0">
        {stats.meta.reverse().map((plugin: Plugin) => {
